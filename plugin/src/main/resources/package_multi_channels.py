...
if is_enable_jiagu == 'true':
    jiagu_apk_path = commands.getoutput("find {base_dir} -name '*jiagu_sign.apk'".format(base_dir=base_dir))
...
def packagingApk(p_channels, p_ex_channel_ids, p_base_apk, p_default_apk_name):
    for channel in data:
        ...
        commands.getoutput("cp {base}/{base_apk} {new_apk}".format(base=base_dir, new_apk=apk_name, base_apk=p_base_apk))
...
if jiagu_base_apk and len(jiagu_base_apk) > 0:
    commands.getoutput("find {base} -type f -not -name '{base_apk}' -not -name '{jiagu_base_apk}' -name '*.apk' -delete".format(base=base_dir, jiagu_base_apk=jiagu_base_apk, base_apk=base_apk))
